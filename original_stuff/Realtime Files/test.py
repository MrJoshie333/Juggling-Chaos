...
# Define a function to handle program termination and save plots
def save_plots(signal, frame):
    print("Saving plots before termination...")
    ...
    # Save the first Fourier Transform plot
    ...
    # Perform a second Fourier Transform on accumulated data (y_positions)
    accumulated_fft_result = np.fft.fft(y_positions)
    accumulated_fft_amplitude = np.abs(accumulated_fft_result)
    fig3 = plt.figure(figsize=(10, 6))
    ax3 = fig3.add_subplot(111)
    ax3.plot(np.fft.fftshift(freqs), np.fft.fftshift(accumulated_fft_amplitude), label="Accumulated Fourier Transform",
             color='red')
    ax3.set_xlabel('Frequency (Hz)')
    ax3.set_ylabel('Amplitude')
    ax3.set_title('Fourier Transform of Accumulated y-Position Over Time')
    ax3.grid(True)
    ax3.legend()
    fig3.savefig('accumulated_fourier_transform.png')  # Save accumulated Fourier Transform plot
    print("Accumulated Fourier transform plot saved as 'accumulated_fourier_transform.png'.")
...
# Set up the signal handler for termination (e.g., pressing the red stop button)
signal.signal(signal.SIGINT, save_plots)
...
# Second plot: Fourier transform of the y-position
...
while True:
    ...
    if len(y_positions) > 0:
        ...
        # Perform Fourier transform on the y-positions
        if len(y_positions) > 1:
            ...
            line2.set_xdata(np.fft.fftshift(freqs))  # Shift frequencies for better visualization
            line2.set_ydata(np.fft.fftshift(fft_amplitude))  # Shift amplitude for visualization
...
# In case the loop is exited gracefully, save the plots
save_plots(None, None)
