...
# Plot trajectories with matplotlib
plt.figure(figsize=(10, 6))
for key, traj in trajectories.items():
    if traj:  # Check if there are any points
        traj = np.array(traj)
        plt.plot(traj[:, 0], traj[:, 1], label=f'Ball {key}')

plt.gca().invert_yaxis()  # Invert y-axis to match image coordinates
plt.title('Trajectories of Juggling Balls')
plt.xlabel('X Coordinate (pixels)')
plt.ylabel('Y Coordinate (pixels)')
plt.legend()
plt.grid(True)
plt.savefig('juggling_trajectory.png')
plt.show()
