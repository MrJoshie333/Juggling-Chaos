...
def main():
    ...
    # Switch to non-GUI backend for thread safety
    matplotlib.use('Agg')

    with concurrent.futures.ThreadPoolExecutor(max_workers=4) as executor:
        # Submit tasks to the thread pool
        future_to_video = {
            executor.submit(process_single_video, path, i, len(paths_to_process), PROCESS_VIDEOS, OUTPUT_DIR): path
        ...
        for future in concurrent.futures.as_completed(future_to_video):
            try:
                ...
                print(f"Error processing video: {future_to_video[future]} - {e}")

    # Process results (e.g., save ratio plots)
    all_l_f = [result["l_f"] for result in all_results if result["l_f"] is not None]
    all_l_u = [result["l_u"] for result in all_results if result["l_u"] is not None]
    point_appearance = [(result["color"], result["label"]) for result in all_results]

    print("Processing complete!")
    print(f"Time taken: {time.time() - start_time:.2f} seconds")

    matplotlib.use('TkAgg')  # Switch back to GUI backend to display ratio plots
...
def process_single_video(path, index, total, process_videos, output_dir):
    output_name = path.split('/')[-1].split('.')[0]

    # Processing logic (moved from main)
    if process_videos:
        print(f"Processing video {index + 1}/{total}: {path.split('/')[-1]}")
        ...
        print(f"Loading data from text files for video {index + 1}/{total}: {path.split('/')[-1]}")
        base_name = path.split("/")[-1].split(".")[0]
        text_path = os.path.join(output_dir, "text_data", f"{base_name}_peaks_valleys.txt")
    ...
    print("Finished processing", path.split("/")[-1])

    return {
        "l_f": l / f if f != 0 else None,
        "l_u": l / u if u != 0 else None,
        "color": "red" if "josh" in path else "blue",
        "label": "J" if "josh" in path else "S",
    }
